...
<div class="container">
  <div class="text-center">
  	<button type="button" onclick="location.href='Schedule.sc?yy=${yy-1}&mm=${mm}';" class="btn btn-info btn-sm" title="이전년도">◁</button>  <!-- 값을 넘겨주고 자신의 위치로 -->
  	...
  	<button type="button" onclick="location.href='Schedule.sc?yy=${yy+1}&mm=${mm}';" class="btn btn-info btn-sm" title="다음년도">▷</button>
  	<button type="button" onclick="location.href='Schedule.sc';" class="btn btn-success btn-sm" title="오늘날짜"><i class="fa-solid fa-house"></i></button>
  </div>
  <br>
  <div class="text-center">
  		...
  		<tr>
  			...
  			<c:forEach begin="1" end="${lastDay}" varStatus="st">
  				...
  				<td id="td${cell}" ${todaySw==1 ? 'class=today' : ''} style="text-align:left;vertical-align:top;height:90px">
  					<c:set var="ymd" value="${yy}-${mm+1}-${st.count}" />  <!-- 너무 길면 변수로 빼기 -->
  				...
  				</td>  <!-- 7개 되면 줄바꿈 // 오늘 날짜가 맞으면 class에 today로 줌 // class="text-left" 부트스트랩 명령 -->
