...
<div class="container">
  <div class="text-center">
  	<button type="button" onclick="location.href='Calendar2.st?yy=${yy-1}&mm=${mm}';" class="btn btn-secondary btn-sm" title="이전년도">◁</button>  <!-- 값을 넘겨주고 자신의 위치로 -->
  	<button type="button" onclick="location.href='Calendar2.st?yy=${yy}&mm=${mm-1}';" class="btn btn-info btn-sm" title="이전월">◀</button>
  	<font size="5">${yy}년 ${mm+1}월</font>
  	<button type="button" onclick="location.href='Calendar2.st?yy=${yy}&mm=${mm+1}';" class="btn btn-info btn-sm" title="다음월">▶</button>
  	<button type="button" onclick="location.href='Calendar2.st?yy=${yy+1}&mm=${mm}';" class="btn btn-secondary btn-sm" title="다음년도">▷</button>
  	<button type="button" onclick="location.href='Calendar2.st';" class="btn btn-success btn-sm" title="오늘날짜"><i class="fa-solid fa-house"></i></button>
  </div>
  <br>
  <div class="text-center">
  	<table class="table table-bordered" style="height:450px">  <!-- 크기 따로 안주면 전체의 90퍼 -->
  		...
  		<tr>
  			...
  			<c:forEach var="prevDay" begin="${prevLastDay-(startWeek-2)}" end="${prevLastDay}">  <!-- 막날이 30이면 3일 28부터 해야함 -2해줌 -->
  				<td style="font-size:0.9em;color:#ccc;text-align:left;">${prevYear}-${prevMonth+1}-${prevDay}</td>  <!-- 1em이 16px -->
  			</c:forEach>
  			...
  			<c:forEach begin="1" end="${lastDay}" varStatus="st">
  				...
  				<c:if test="${cell % 7 == 0}"></tr><tr></c:if>  <!-- st.count 쓰면 좋은데 이미 위에서 써버림(7개가 되면 줄바꿈) -->
  				<c:set var="cell" value="${cell + 1}" />
  			</c:forEach>
  			
  			<!-- 마지막 이후를 다음달의 시작일자로부터 채워준다. -->
  			<c:if test="${(cell-1) % 7 != 0}">
  				<c:forEach begin="${nextStartWeek}" end="7" varStatus="st">  <!-- 31이 4이면 5부터 2개 -->
  					<td style="font-size:0.9em;color:#ccc;text-align:left;">${nextYear}-${nextMonth+1}-${st.count}</td>  <!-- 1,2,3 이렇게 된까 count -->
  				</c:forEach>
  			</c:if>
  		</tr>
  	</table>
