...
<div class="container">
  ...
  <div class="text-right">
  	<button type="button" onclick="location.href='jstl1.jsp';" class="btn btn-success">JSTL core라이브러리</button>  <!-- 기본(표준)이 button이고 javascript: 안써도 됨 -->
  	<button type="button" onclick="location.href='jstl3.jsp';" class="btn btn-primary">JSTL 반복문</button>
  </div>
  <hr/>
...
%>
	...
	* 5단 *<br/>
	...
	<div>
		<p><input type="number" name="su1" id="su1" value="1" class="form-control" required/></p>
		<p><input type="number" name="su2" id="su2" value="1" class="form-control" required/></p>
		<p><input type="number" name="col" id="col" value="1" class="form-control" required/></p>
		<c:if test="${su1>su2}">
			alert("시작단에 더 큰 수를 입력하셨습니다! 다시 입력해 주세요");
		</c:if>
		<c:if test="${su1<=su2}">
		<c:forEach var="i" begin="su1" end="su2">
			* ${i}단 *<br/>
			<c:forEach var="j" begin="1" end="9">
				<tr>
					<td>${i} * ${j} = ${i*j}</td>
		...
		<p><input type="button" value="구구단 출력" onclick="forCheck()" class="btn btn-success form-control" /><p>
	</div>
	<br/>
